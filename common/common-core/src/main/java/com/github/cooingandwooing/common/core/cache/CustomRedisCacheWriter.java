...
package com.github.cooingandwooing.common.core.cache;
...
public class CustomRedisCacheWriter implements RedisCacheWriter {
		...
		execute(name, connection -> {
			...
			if (shouldExpireWithin(ttl)) {
				connection.set(key, value, Expiration
						.from(ttl.toMillis(), TimeUnit.MILLISECONDS), RedisStringCommands.SetOption.upsert());
			}
			else {
	...
	private void checkAndPotentiallyWaitUntilUnlocked(String name, RedisConnection connection) {
		...
		try {
			...
			throw new PessimisticLockingFailureException(String
					.format("Interrupted while waiting to unlock cache %s", name),
					ex);
		}
	}
